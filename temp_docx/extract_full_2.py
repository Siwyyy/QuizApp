...
def extract_quiz(filepath):
    ...
    quiz = {"title": "Baza Farmakognozja (Z pytaniami ABCD)", "questions": []}
...
# Clean empty explanations
valid_questions = []
for q in quiz_data['questions']:
    # Let's count how many valid questions we have, only those where AT LEAST ONE option is green!
    has_correct = any(opt['isCorrect'] for opt in q['options'])
    
    # User said "weź te, które są A,B,C,D" -> must have options.
    # The user said "za poprawną odpowiedź weź tą pokolorowaną na zielono".
    # I should include the question if it has multiple options. If it doesn't have a correct option marked, 
    # it might be due to missing coloring, but I should probably still include it to not skip valid ABCD questions.
    valid_q = {
        "text": q['text'],
        "options": q['options']
    }
